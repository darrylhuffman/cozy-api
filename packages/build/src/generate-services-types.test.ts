...
describe("generateServicesTypes (fixture-based)", () => {
  // Use the runtime's fixture which has a lorien.config.ts exporting `db`
...
describe("generateServicesTypes (no config)", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "lorien-types-"));
  });
  ...
  it("returns null path when no lorien.config.ts exists", async () => {
