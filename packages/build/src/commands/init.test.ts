...
describe("runInit", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "cozy-init-"))
  })
  ...
  it("writes AGENTS.md when none exists", async () => {
    ...
    expect(content).toMatch(/cozy-api/)
  ...
  it("overwrites with --force", async () => {
    ...
    expect(readFileSync(join(dir, "AGENTS.md"), "utf-8")).toMatch(/cozy-api/)
  })

  it("uses the directory basename as the project name", async () => {
    // dir is /tmp/cozy-init-xxxxxx — basename starts with cozy-init-
