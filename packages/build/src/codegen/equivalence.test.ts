...
import { testWorkflow } from "@cozy/runtime/testing"
...
describe("equivalence: interpreter == codegen", () => {
  it("POST /users with valid credentials produces identical responses", async () => {
    ...
    const parseCredentials = (
      await import(pathToFileURL(join(basicApiRoot, "nodes", "parse-credentials.ts")).href)
    ).default
    const saveUser = (await import(pathToFileURL(join(basicApiRoot, "nodes", "save-user.ts")).href))
      .default
    ...
    // --- Codegen side ---
    // The generated .gen.ts imports `../../../cozy.config.js` and uses
    ...
    const configMod = await import(pathToFileURL(join(basicApiRoot, "cozy.config.ts")).href)
    const config = configMod.default as {
      services: { db: { createUser: (e: string, p: string) => Promise<unknown> } }
    }
