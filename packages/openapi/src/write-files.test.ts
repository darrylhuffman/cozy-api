...
describe("writeGeneratedFiles", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "lorien-write-"));
  });
  ...
  const operationFile: GeneratedFile = {
    ...
    source:
      "// lorien-openapi: generated from operation `getPet`.\nexport default {}\n",
  };
  ...
  it("writes files when they don't exist", async () => {
    ...
    expect(readFileSync(join(dir, "get-pet.ts"), "utf-8")).toContain(
      "lorien-openapi: generated",
    );
  });
  ...
  it("overwrites generated files (have marker) on re-import", async () => {
    writeFileSync(
      join(dir, "get-pet.ts"),
      "// lorien-openapi: generated from earlier\n// outdated content\n",
    );
    const result = await writeGeneratedFiles([operationFile], dir, {
      quiet: true,
    });
    expect(result.written).toEqual(["get-pet.ts"]);
    expect(readFileSync(join(dir, "get-pet.ts"), "utf-8")).toBe(
  ...
  it("preserves _client.ts on re-import regardless of marker", async () => {
    ...
    expect(readFileSync(join(dir, "_client.ts"), "utf-8")).toContain(
      "user customized",
    );
  });
  ...
  it("creates parent directories as needed", async () => {
    ...
    const result = await writeGeneratedFiles([operationFile], nestedDir, {
      quiet: true,
    });
    expect(result.written).toEqual(["get-pet.ts"]);
    expect(readFileSync(join(nestedDir, "get-pet.ts"), "utf-8")).toContain(
      "lorien-openapi",
    );
  });
});
