...
describe("validateOpenApi", () => {
  ...
  it("accepts 3.1", () => {
    expect(() =>
      validateOpenApi({ openapi: "3.1.0", info: { title: "x", version: "1" }, paths: {} }),
    ).not.toThrow()
  })
  ...
  it("rejects 2.x (Swagger)", () => {
    expect(() => validateOpenApi({ openapi: "2.0", paths: {} })).toThrow(/3\.x/)
  })
...
describe("loadOpenApiSpec", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "cozy-oas-"))
  })
  ...
  it("errors with file context when file doesn't exist", async () => {
    await expect(loadOpenApiSpec(join(dir, "missing.json"))).rejects.toThrow(OpenAPIError)
    await expect(loadOpenApiSpec(join(dir, "missing.json"))).rejects.toThrow(/Failed to read/)
  })
})
